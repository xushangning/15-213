 ...
 */
int tmin(void) {
  /* exploit the binary representation of TMin being 1 followed by 31 zeroes */
  return 1 << 31;
...
int allOddBits(int x) {
  /* generate 0xAAAAAAAA with rightshifts and addition, clear even bits of x
   * with AND, and finally xor with 0xAAAAAAAA to test for odd bits.
   */
  int oddBits = (0xAA << 8) + 0xAA;
  oddBits = (oddBits << 16) + oddBits;    // now oddBits == 0xAAAAAAAA
  return !(~x & oddBits);
}
/* 
 * negate - return -x 
...
int logicalNeg(int x) {
  /* OR every bit of x */
  x = x | x >> 16;  // upper 16 bits ORed with the lower 16 bits
  x = x | x >> 8;
  x = x | x >> 4;
  x = x | x >> 2;
  // The ORed result is stored in the last bit of (x | x >> 1).
  return (x | x >> 1) & 1 ^ 1;
}
